  ...
  return (
    <>  
      <article className={`${Styles.StaticCoursesContent}`}>
        ...
        <p className={`${Styles.M_Top}`}>Choose from over 220,000 online video courses with new additions published every month</p>
      ...
      //   {CourseIntro.map((item, index) => (
      //     <div key={index} className={`${selectedTab === index ? "" : Styles.Hidden}`}>
      //       <h3 className={`${Styles.M_Top} ${Styles.CoursesH4}`}>{item.header}</h3>
      //       <p className={`${Styles.M_Top} ${Styles.CoursesH4} ${Styles.CourseDesc}`}>{item.description}</p>
      //       <button className={`${Styles.M_Top} ${Styles.ExploreBtn} ${Styles['M_Bottom']} ${Styles.CoursesH4}`}>
      //         <a className={`${Styles.ExploreBtnAnchor}`} href={`https://www.udemy.com/topic/${item.urlkey}/`}>Explore {item.TabName}</a>
      //       </button>
      //       <h4 className={`${Styles.CoursesH4} ${Styles['M_Top']} ${Styles['M_Bottom']}`}>{item.title}</h4>
      //       <div className="App">
